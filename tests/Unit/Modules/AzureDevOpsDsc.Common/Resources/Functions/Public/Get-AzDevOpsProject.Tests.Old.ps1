    ...
    Describe 'AzureDevOpsDsc.Common\Get-AzDevOpsProject' -Tag 'GetAzDevOpsProject' {
        ...
        Context 'When called with valid parameters' {
            ...
            Context 'When called with no "ProjectId" parameter and no "ProjectName" parameter' {
                ...
                It 'Should call "Get-AzDevOpsApiResource" function once per project present/available + 1 initial call - "<ApiUri>", "<Pat>"' -TestCases $testCasesValidApiUriPatCombined {
                    param ([string]$ApiUri, [string]$Pat)

                    Get-AzDevOpsProject -ApiUri $ApiUri -Pat $Pat | Out-Null
                    Should -Invoke Get-AzDevOpsApiResource -ModuleName $script:subModuleName -Times $($mockProjects.Count + 1) -Exactly -Scope It
                }
